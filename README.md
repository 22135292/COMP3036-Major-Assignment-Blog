# Full Stack Development Major Assignment — Blog Application

This repository contains a complete two-application blog platform: a public reader experience on port `3001` and an authenticated content studio on port `3002`.

## Major assignment enhancements

The project implements all four assessed enhancements with matching Playwright coverage:

1. **Rich text editing** — authors can switch between Markdown and a Tiptap rich-text editor.
2. **Cloud image uploads** — authenticated authors can upload post artwork to Cloudinary and preview it before saving.
3. **Pagination** — public and admin post listings use server-side pagination with stable URL parameters.
4. **Nested comments** — readers can post comments and reply in threaded conversations stored in the database.

The refreshed responsive interface uses an editorial visual system for readers and a purpose-built dashboard layout for authors. It supports desktop and mobile navigation, light/dark reader themes, filtering, search, likes, views, categories, tags, archives, post visibility and create/update workflows.
...
`.github/workflows/ci.yml` installs dependencies, prepares the SQLite database, checks TypeScript, runs tests, builds both applications and runs Playwright E2E tests. Add the three Cloudinary repository secrets before enabling the upload E2E test: `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, and `CLOUDINARY_API_SECRET`.
...
Open `http://localhost:3001` for the public blog and `http://localhost:3002` for the admin studio.

---

## Original Assignment 2 specification
