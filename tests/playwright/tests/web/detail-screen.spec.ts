...
test.describe("DETAIL SCREEN", () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      // DETAIL SCREEN > Verify all elements on the detail screen
  ...
  test(
    ...
    async ({ page }) => {
      ...
      // make sure the POST api call success, mean that toast message is visible
      await expect(page.getByText("You liked this post")).toBeVisible();

      await page.goto("/post/boost-your-conversion-rate");
      
      // wait for the GET api call
      await page.waitForTimeout(2000);

      await expect(page.getByText("4 likes")).toBeVisible();
      await page.getByTestId("like-button").click();

      // make sure the DELETE api call success, mean that toast message is visible
      await expect(page.getByText("You unliked this post")).toBeVisible();
      await expect(page.getByText("3 likes")).toBeVisible();
    },
  );
});
