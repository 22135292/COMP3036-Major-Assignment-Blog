...
test.describe("ADMIN LIST SCREEN", () => {
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      await expect(userPage).toHaveURL("/search?q=Boost");
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      await expect(userPage).toHaveURL("/search?tag=Front");
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      await expect(userPage).toHaveURL("/search?date=2022-01-01");

      await expect(await userPage.locator("article").count()).toBe(2);
      await expect(
        userPage.getByText("Database scaling made easy"),
      ).toBeVisible();
      await expect(
        userPage.getByText("How to deploy with Docker"),
      ).toBeVisible();
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      await userPage.getByLabel("Filter by Tag:").fill("Linux");
      await expect(userPage).toHaveURL("/search?tag=Linux");

      await userPage
        .getByLabel("Filter by Date Created:")
        .pressSequentially("01012022");
      await expect(userPage).toHaveURL("/search?tag=Linux&date=2022-01-01");
      await expect(await userPage.locator("article").count()).toBe(1);
      await expect(
        userPage.getByText("How to deploy with Docker"),
      ).toBeVisible();
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      // title-asc (Default is Title Asc, so no action needed, or ensure reset if testing repeatedly)
      // Current state: Title Asc
      ...
      // title-desc
      await userPage.getByRole("button", { name: "Sort by Title" }).click();
      ...
      // date-asc
      // Clicking "Sort by Date" switches to Date and defaults to Asc
      await userPage.getByRole("button", { name: "Sort by Date" }).click();
      articles = await userPage.locator("article").all();
      ...
      // date-desc
      // Clicking "Sort by Date" again toggles to Desc
      await userPage.getByRole("button", { name: "Sort by Date" }).click();
      articles = await userPage.locator("article").all();
  ...
  test(
    ...
    async ({ userPage }) => {
      ...
      await expect(article.getByText("#Front-End")).toBeVisible();
      await expect(article.getByText("#Dev Tools")).toBeVisible();
      await expect(article.getByText("Posted on 16 Dec 2024")).toBeVisible();
